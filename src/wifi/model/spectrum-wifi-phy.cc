        ...
        SpectrumWifiPhy::GetTypeId (void)
        {
            static TypeId tid = TypeId ("ns3::SpectrumWifiPhy")
                ...
                .AddTraceSource ("SignalTransmission",
                        "Signal transmission",
                        MakeTraceSourceAccessor (&SpectrumWifiPhy::m_signalTx),
...
switchChannel:
    ...
    void
        SpectrumWifiPhy::StartRx (Ptr<SpectrumSignalParameters> rxParams)
        {
            ...
            uint32_t selfNodeId = 0;
            selfNodeId = m_device->GetNode ()->GetId ();//rxParams->txPhy->GetDevice ()->GetNode ()->GetId ();
            if (rxParams->txPhy)
            {
                senderNodeId =  rxParams->txPhy->GetDevice ()->GetNode ()->GetId ();
            }
            NS_LOG_DEBUG (selfNodeId << "Received signal from " << senderNodeId << " with unfiltered power " << WToDbm (Integral (*receivedSignalPsd)) << " dBm");
            ...
            if (wifiRxParams == 0)
            {
                ...
                // TODO: Ratnesh - Should we store the information about end of our own transmission
                // It might be necessary so as to provide DIFS gap between TXOP
                // if(selfNodeId != senderNodeId){
                    ...
                    case SpectrumWifiPhy::IDLE:
                        ...
                        else
                        {
                            NS_LOG_DEBUG ("drop packet because signal power too Small (" <<
                                    rxPowerW << ") " << m_random->GetValue () << " " <<m_frameSyncErrorRateModel->GetFrameSyncErrorRate (RatioToDb (m_interference.CalculateAvgSyncFieldSnir (event))));
                            NotifyRxDrop (packet);
                            m_plcpSuccess = false;
                            goto maybeCcaBusy;
                        }
...
maybeCcaBusy:
            ...
            {
                // if(selfNodeId != senderNodeId){
                    NS_LOG_DEBUG ("Calling SwitchMaybeToCcaBusy for " << delayUntilCcaEnd << " at time " << Simulator::Now () << " " << m_ccaMode1ThresholdW);
                    m_state->SwitchMaybeToCcaBusy (delayUntilCcaEnd);
                // }
            }
            else
            {
                NS_LOG_DEBUG ("Energy is lower than CCA threshold : " << delayUntilCcaEnd << " at time " << Simulator::Now () << ", Threshold: " << WToDbm(m_ccaMode1ThresholdW));
            }
        }
    ...
    void
        SpectrumWifiPhy::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble, enum mpduType mpdutype)
        {
            ...
            NS_LOG_DEBUG ("data rate used : "  <<dataRate500KbpsUnits*500 << " Kbps");
            // Log tx parameter
            m_signalTx (dataRate500KbpsUnits);
