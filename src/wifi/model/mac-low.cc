...
namespace ns3 {
    ...
    //TODO: Ratnesh
    void
        MacLow::StartLaaCtsToSelfTx (Time duration)
        {
            NS_LOG_FUNCTION (this << duration);
            //Send a CTS-to-self reserving the channel for a given duration
        ...
        MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, WifiPreamble preamble, bool ampduSubframe)
        {
            ...
            else if (hdr.GetAddr1 () == m_self)
            {
                ...
                else if (hdr.IsData () || hdr.IsMgt ())
                {
                    ...
                    {
                        NS_LOG_DEBUG ("rx unicast/sendAck from=" << hdr.GetAddr2 () << " " << hdr.GetDuration ());
                        NS_ASSERT (m_sendAckEvent.IsExpired ());
                        m_sendAckEvent = Simulator::Schedule (GetSifs (),
                                &MacLow::SendAckAfterData, this,
                                hdr.GetAddr2 (),
...
rxPacket:
    ...
    void
        MacLow::SendCtsToSelf (void)
        {
            ...
            // XXX: Ratnesh : if used as wifi listener then dont trasnmit anything after cts-to-self
            NS_LOG_DEBUG ("CTS transmit duration :" << txDuration);
            if (m_laawifi == true)
                return;
    ...
    void
        MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
        {
            ...
            duration -= GetSifs ();
            NS_LOG_DEBUG ("****Test : " << duration<< " " <<GetAckDuration (ackTxVector)<< " "<<GetSifs() );
            // TODO: Ratnesh : Disabling this assert
            // NS_ASSERT_MSG (duration >= MicroSeconds (0), "Please provide test case to maintainers if this assert is hit."  << source << " " << duration<< " " <<GetAckDuration (ackTxVector)<< " "<<GetSifs() );
            if (duration < MicroSeconds(0))
            {
                NS_LOG_DEBUG ("**************************** PROBLEM : " << duration<< " " <<GetAckDuration (ackTxVector)<< " "<<GetSifs() );
