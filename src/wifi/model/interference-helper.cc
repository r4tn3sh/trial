...
InterferenceHelper::AddForeignSignal (Time duration, double rxPowerW)
{
  ...
  // of signal
  NS_LOG_FUNCTION (this << " " << duration << " " << rxPowerW);
...
InterferenceHelper::GetEnergyDuration (double energyW)
{
  ...
  for (NiChanges::const_iterator i = m_niChanges.begin (); i != m_niChanges.end (); i++)
    {
      // // FIXME: not sure about the role of m_firstPower
      // // large value of m_firstPower seems to mess things
      noiseInterferenceW += i->GetDelta ();
      end = i->GetTime ();
      NS_LOG_DEBUG ("noiseInterferenceW=" << i->GetDelta () << ", end= " << end << ", now=" << now << ", m_firstPower=" << m_firstPower);
      if (end < now)
        {
          continue;
        }
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG (event->GetStartTime () << " " << event->GetRxPowerW () << " " << event->GetEndTime ());
  Time now = Simulator::Now ();
  if (!m_rxing)
    {
