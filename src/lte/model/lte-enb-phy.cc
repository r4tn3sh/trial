...
namespace ns3 {
        ...
        LteEnbPhy::GetTypeId (void)
        {
            static TypeId tid = TypeId ("ns3::LteEnbPhy")
                ...
                .AddTraceSource ("SignalTransmission",
                        "Signal transmission",
                        MakeTraceSourceAccessor (&LteEnbPhy::m_signalTx),
                        "ns3::LteEnbPhy::SignalTransmissionCallback")
    ...
    void
        LteEnbPhy::StartSubFrame (void)
        {
            ...
            */
            ...
            // NS_LOG_INFO ("-----sub frame " << m_nrSubFrames << "-----" << m_grantTimeout << " " <<  m_ttiBegin << " " << !m_isWaitingForChannelAccessGrant << " " << IsThereData());
            NS_LOG_INFO ("-----sub frame " << m_nrSubFrames << "-----" << m_grantTimeout << " " <<  m_ttiBegin);
            ...
            else  // if there is channel access manager
            {
                ...
                else  // implementation 2 - without 2ms delay, scheduler is not being stopped
                {
                    ...
                    if (!IsNonEmptyCtrMessage() && !IsNonEmptyPacketBurst())
                    {
                        NS_LOG_INFO ("-----Data not ready to be transmitted----- ");
                        GetControlMessages ();
                        GetPacketBurst();
                    ...
                    else
                    {
                        ...
                        NS_LOG_INFO ("-----sub frame " << m_nrSubFrames << "-----" << m_grantTimeout << " " <<  m_ttiBegin << " " << Seconds(GetTti()));
                        ...
                        {
                            NS_LOG_INFO ("-----Continue waiting for grant------");
                        if (m_grantTimeout > m_ttiBegin)
                            if(m_dropPackets)
                            {
                ...
                while (it != ctrlMsg.end ())
                {
                    ...
                    if (msg->GetMessageType () == LteControlMessage::DL_DCI)
                    {
                        ...
                        for (uint8_t i = 0; i < dci->GetDci ().m_mcs.size (); i++)
                        {
                            ...
                            NS_LOG_DEBUG ("MCS : "<<uint32_t(params.m_mcs));
                            m_signalTx (uint32_t(params.m_mcs));// XXX: Ratnesh : trace for MCS entry
                    ...
                    else if (msg->GetMessageType () == LteControlMessage::RAR)
                    {
                        ...
                        for (std::list<RarLteControlMessage::Rar>::const_iterator it = rarMsg->RarListBegin (); it != rarMsg->RarListEnd (); ++it)
                        {
                            ...
                            dci.m_ndi = 1;
                            NS_LOG_DEBUG ("DCI MCS : "<<int(dci.m_mcs));
    ...
    void
        ...
        {
            NS_LOG_FUNCTION (this << grantDuration);
