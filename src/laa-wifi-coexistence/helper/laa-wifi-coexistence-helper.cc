  ...
  for (NetDeviceContainer::Iterator i = enbDevices.Begin (); i != enbDevices.End (); ++i)
    {
      ...
      Ptr<AdhocWifiMac> wifiMac = monitor->GetObject<WifiNetDevice> ()->GetMac ()->GetObject<AdhocWifiMac>();
      // Ptr<MacLow> macLow = CreateObject<MacLow> ();
      Ptr<MacLow> macLow = wifiMac->m_low;
      // macLow->SetPhy(wifiPhy);
      ...
      if (m_channelAccessManagerFactory.GetTypeId ().GetName () == "ns3::BasicLbtAccessManager")
      ...
      else if (m_channelAccessManagerFactory.GetTypeId ().GetName () == "ns3::LbtAccessManager")
        {
          ...
          // XXX: No need to use following function now, handling directly from script
